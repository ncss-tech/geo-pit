...
def createFGDB(regionChoice,outputFolder):
    ...
    try:
        ...
        # CONUS - USA Contiguous Albers Equal Area Conic USGS version NAD83
            ...
            xmlFile = os.path.dirname(sys.argv[0]) + os.sep + "RTSD_XMLWorkspace_CONUS.xml"
            newName = "RTSD_MLRA_" + str(regionChoice[regionChoice.find(" ")+1:]) + "_" + FY
...
# i.e. alias name for MUPOLYGON = Region 10 - Mapunit Polygon
    ...
    try:
        ...
        if arcpy.Exists(os.path.join(fdPath,'MUPOLYGON')):
            arcpy.AlterAliasName(os.path.join(fdPath,'MUPOLYGON'), "RTSD MLRA R" + regionNumber + " - Mapunit Polygon")  #MUPOLYGON
            aliasUpdate += 1

        if arcpy.Exists(os.path.join(fdPath,'SAPOLYGON')):
            arcpy.AlterAliasName(os.path.join(fdPath,'SAPOLYGON'), "RTSD MLRA R" + regionNumber + " - Survey Area Polygon")  #SAPOLYGON
            aliasUpdate += 1
...
if not arcpy.Exists(regionalTable):
    raise MyError, "\n" + "MLRA Master Table is missing from " + os.path.dirname(sys.argv[0])
