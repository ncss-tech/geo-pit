...
```{r}
# Set parameters
mlrassoarea <- paste0("11-", c("ATL", "AUR", "MAN", "CLI", "FIN", "GAL", "IND", "JUE", "SPR", "UNI", "WAV", "LOHR"))
mlraoffice <- c("11")
...
gd <- {
  ...
  within(., {
    ...
    file_path = paste0("D:/geodata/project_data/R", folder, paste0("/", variable, resolution, "_", folder))
...
files <- list.files("D:/geodata/elevation/ned/tiles/ArcGrid/19", pattern = ".zip$")

idx <- ! r11$FILE_ID %in% gsub(".zip", "", files)
r11[idx, ] ->.;
split(r11, r11$FILE_ID) ->.;
lapply(., function(x) {
  cat("downloading", x$FILE_ID, as.character(Sys.time()), "\n")
  download_ned_tile("13", x$UL_LAT, abs(x$UL_LON), "D:/geodata/elevation/ned/tiles/ArcGrid")
})
...
files <- list.files("D:/geodata/elevation/ned/tiles/ArcGrid/13", pattern = ".zip$")

idx <- !conus$idx %in% lf$tiles
conus ->.; #[idx, ] ->.;
split(., .$idx) ->.;
lapply(., function(x) {
  cat("getting", x$idx, "\n")
  download_ned_tile("1", x$y, abs(x$x), "D:/geodata/elevation/ned/tiles/ArcGrid")
  })
...
```{r}
nlcd <- "D:/geodata/land_use_land_cover/nlcd_2011_landcover_2011_edition_2014_03_31.img"
# sso_dsn <- read_sf(dsn = "D:/geodata/soils/MLRA_Soil_Survey_Areas_Dec2015.shp", layer = "MLRA_Soil_Survey_Areas_Dec2015")
sso_nlcd <-  subset(gd, variable == "nlcd")

# subset(gd, folder == "11-LOHR" & variable == "nlcd") ->.;
split(., .$file_path) ->.;
lapply(., function(x) {
  crop(input = nlcd, output = x$file_path, sso_dsn = r11 %>% st_transform("+init=epsg:5070"), crsarg = crsarg)
  })
...
sso <- "11-LOHR"
# gd_sub <- subset(gd, variable == "ned" & resolution == "09d" & folder == "11-LOHR")
sso_ned <- subset(gd, variable == "ned" & resolution == "09d" & folder == sso)
sso_ned_sub <- subset(sso_ned, mlrassoarea == sso)
idx <- list.files("D:/geodata/elevation/ned/tiles/ArcGrid/13", full.names = TRUE)
...
# ned_conus <- lapply(ned_conus, function(x) filename = rast(paste0(x, "/w001001.adf")))
# ned_conus$filename <- "D:/geodata/elevation/ned/ned_1d_conus.tif"
# m  <- do.call(merge, ned_conus)

mosaic(paste0(ned_conus, "/w001001.adf"), "D:/geodata/elevation/ned/ned_1as_conus.tif", "Float32", c("BIGTIFF=YES"), -99999)
...
```{r}
warp("D:/geodata/project_data/R11-LOHR/ned09d_11-LOHR.tif", "D:/geodata/project_data/R11-LOHR/ned10m_11-LOHR.tif", "D:/geodata/project_data/R11-LOHR/nlcd30m_11-LOHR_lulc2011.tif", 10, "bilinear", CRSargs(CRS("+init=epsg:4326")), crsarg, "Float32", -99999, c("BIGTIFF=YES"))


# Region 11
# For some reason warping the ned_1as_r11.tif takes forever. Therefore I cut it up prior warping.
sapolygon <- read_sf("D:/geodata/soils/SSURGO_R11_FY19.gdb", layer = "SAPOLYGON") %>% st_transform("+init=epsg:5070")
tiles     <- read_sf("D:/geodata/elevation/ned/tiles_3_overlapping.shp", layer = "tiles_3_overlapping")
...
lapply(., function(x) {
  ...
  warp("C:/geodata/elevation/ned/ned_13as_r11.tif",
       paste0("D:/geodata/elevation/ned/ned_10m_r11_", x$tile, ".tif"), 
       paste0("D:/geodata/project_data/11REGION/nlcd30m_11R_lulc_", x$tile, ".tif"), 
       10, "bilinear", 
...
split(tiles, tiles$idx) ->.;
lapply(., function(x) {
  crop("D:/geodata/elevation/ned/ned_30m_conus.tif",
...
# R11 mosaic 30-meter
mosaic(paste0("D:/geodata/elevation/ned/ned_30m_r11_", tiles$idx, ".tif"), "D:/geodata/elevation/ned/ned_30m_r11.tif", "Float32", c("BIGTIFF=YES"), -99999)
...
resample("D:/geodata/project_data/R11-LOHR/ned10m_11-LOHR.tif",
         "D:/geodata/project_data/R11-LOHR/ned30m_11-LOHR.tif",
         "D:/geodata/project_data/R11-LOHR/nlcd30m_11-LOHR_lulc2011.tif",
         30
         )
```
...
subset(gd, folder == "11-LOHR" & var_res == "ned10m") ->.;
split(., .$folder) ->.;
lapply(., function(x) {
...
dem("D:/geodata/elevation/ned/ned_30m_r11.tif", c("TILED=YES", "COMPRESS=DEFLATE", "BIGTIFF=YES"))
