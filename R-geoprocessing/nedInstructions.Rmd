...
gd <- {
  rbind(
    expand.grid(variable   = c("nlcd"),
               resolution  = c("30m"),
               office = c(mlrassoarea, mlraoffice),
  ...
  within(., {
    file_path = paste0("M:/geodata/project_data/R", office, paste0("/", variable, resolution, "_", office))
    file_path = ifelse(grepl("nlcd", file_path), 
                        paste0(file_path, "_lulc2011.tif"),
                        paste0(file_path, ".tif")
                        )
    }) ->.;
  .[order(.$variable, .$resolution, .$office), ] ->.;
}
```
...
subset(test, mlrassoarea == "11-JUE") ->.;
split(., .$FILE_ID) ->.;
lapply(., function(x) {
...
subset(gd, office == "11-JUE" & variable == "nlcd") ->.;
split(., .$file_path) ->.;
lapply(., function(x) {
  batch_crop(nlcd, sso_dsn, x$office, x$file_path, "nearest", crsarg)
  })
```
...
gd_sub <- subset(gd, variable == "ned" & resolution == "09d" & office == "11-JUE")
...
batch_mosaic(sso_ned_grid, gd_sub$file_path, "Float32", c("BIGTIFF=YES"), -99999)
```
