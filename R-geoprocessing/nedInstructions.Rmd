...
# download CONUS
ned_idx   <- read_sf(dsn = ned_dsn, layer = "ned_13arcsec_g")
ned_conus <- subset(ned_idx, UL_LON > -125 & UL_LON < -64 & UL_LAT < 49 & UL_LAT > 21)
ned_conus$id <- 1:nrow(ned_conus)

split(ned_conus, ned_conus$FILE_ID) ->.;
lapply(., function(x) {
  cat("getting", x$id, x$FILE_ID, "\n")
  download_ned_tile("1", x$UL_LAT, abs(x$UL_LON), "D:/geodata/elevation/ned/tiles/grid")
  })
