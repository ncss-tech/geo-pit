...
sso_ned_nodups <- sso_ned[!duplicated(sso_ned$sso_key), ]
...
ned_files <- sso_ned[!duplicated(sso_ned$sso_key), "FILE_ID", drop = TRUE]
files <- list.files("M:/geodata/elevation/ned/tiles/img/")
missing <- remove_dups(files, ned_files)

url_in <- paste0("ftp://rockyftp.cr.usgs.gov/vdelivery/Datasets/Staged/NED/13/IMG/", missing, ".zip")
file_out <- paste0("M:/geodata/elevation/ned/tiles/ArcGrid/", missing, ".zip")
