...
source("C:/Users/stephen.roecker/Documents/geo-pit/trunk/R-geoprocessing/rsagaFunctions.R")
```
...
batch_mosaic(list(elev.sdat), "M:/geodata/project_data/6REGION/ned30m_R6_elev5.tif", nodata = -99999, datatype = "Float32", co = c("BIGTIFF=NO"))


# Create a copy of elev5 to mask---------------------------------------------------------
gdal_translate(
  src_dataset = "M:/geodata/project_data/6REGION/ned30m_R6_elev5.tif",
  dst_dataset = "M:/geodata/project_data/6REGION/ned30m_R6_elev5_masked.tif",
  overwrite = TRUE,
  verbose = TRUE
)
...
for(i in seq(nhd_wb)){
  ...
  gdal_rasterize(
    ...
    dst_filename = "M:/geodata/project_data/6REGION/ned30m_R6_elev5_masked.tif",
...
rsaga.ca(demf, caarea, caheight)
rsaga.twi(slopeR, caarea, wetness)
...
flt.sdat <- sapply(g30[flt], paste0, ".sdat")
